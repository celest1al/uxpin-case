...
import { buttonColor, buttonSize } from 'constants/color.constant'
...
export const ButtonContainer = styled.button<ButtonStyleProps>`
  border: none;
  border-radius: 2px;
  padding: ${props => buttonSize[props?.size || 'default']?.padding};
  font-size: 0.875rem;
  cursor: pointer;
  ${props =>
    props.color &&
    css`
      ...
      font-weight: ${buttonColor[props.color]?.fontWeight};
    `}
`
